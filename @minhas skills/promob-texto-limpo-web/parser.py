...
def process_promob_text(input_text: str, dobradicas: list = None, corredicas: list = None) -> tuple:
    """
    Process Promob text and return formatted output and questions data.

    Args:
        input_text: Raw Promob text
        dobradicas: List of selected dobradiças (optional for first pass)
        corredicas: List of selected corrediças (optional for first pass)

    Returns:
        (formatted_output, questions_data) tuple
    """
    parser = PromobParser()
    ...
    # If choices provided, format output
    if dobradicas is not None or corredicas is not None:
        output = parser.format_output(
            include_dobradica=dobradicas,
            include_corredica=corredicas
        )
        return output, questions_data

    # Otherwise just return questions
